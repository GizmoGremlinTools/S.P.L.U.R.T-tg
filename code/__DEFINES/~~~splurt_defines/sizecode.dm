...
/*
 * # get_size_amp(mob/living/target)
 * Grabs the size of your critter. Applies a reduction of 40% if both legs are missing
*/
/proc/get_size_amp(mob/living/target)
	if(!target)
		CRASH("get_size(NULL) was called")
	if(!istype(target))
		CRASH("get_size() called with an invalid target, only use this for /mob/living!")
	var/datum/dna/has_dna = target.has_dna()
	if(ishuman(target) && has_dna)
		if(target.get_bodypart(BODY_ZONE_L_LEG) == null && target.get_bodypart(BODY_ZONE_R_LEG) == null)
...
/*
 * # COMPARE_SIZES_AMP(mob/living/user, mob/living/target)
 ...
 * - target = /mob/living
*/
#define COMPARE_SIZES_AMP(user, target) abs((get_size_amp(user) / get_size_amp(target)))
