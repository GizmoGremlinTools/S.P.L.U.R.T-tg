...
/datum/element/mob_holder/micro/proc/mob_try_pickup_micro(mob/living/carbon/source, mob/living/carbon/user)
	...
	if(source == user)
		...
		return FALSE
	if(COMPARE_SIZES_AMP(user, source) < 1.2)
	...
	var/time_required = COMPARE_SIZES_AMP(source, user) * 4 SECONDS //Scale how fast the pickup will be depending on size difference
